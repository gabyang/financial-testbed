...
class StockAnalyzer:
    ...
    def get_relevant_articles(self, symbol, window_start, conn, top_n=None):
        ...
        try:
            ...
            cur.execute("""
                SELECT a.symbol, a.title, a.date, a.source, c.chunk_text,
    ...
    def get_top_chunks(self, filing_ids, pct_change, conn, top_n=None):
        ...
        try:
            ...
            query_text = f"stock price {movement_type} of {abs(pct_change):.2f}% factors explanation reasons"
    ...
    def generate_llm_summary(self, chunks, article_chunks, stock_data, company_profile):
        ...
        # Use company profile info if available, otherwise use defaults
        if company_profile:
            company_name = company_profile['companyName']
        ...
        - Current Window Avg Close: ${avg_close if avg_close != 'N/A' else 'N/A'}
        - Percent Change: {pct_change:.2f}% {'decrease' if pct_change < 0 else 'increase'}
        - Window: {window} starting {date}
        
        Analyze the following SEC filing & news chunks and explain the factors that likely contributed to this price movement:
        ...
        prompt += "\nBased on the company profile, stock price data, SEC filing excerpts, and news articles, provide a comprehensive analysis of:"
        prompt += "\n1. News articles that reflect the change in stock price"
        prompt += "\n2. Financial results and metrics that impacted investor sentiment. Provide the date and statistic."
        prompt += "\n3. Market conditions or industry trends that affected the company"
    ...
    def save_summary(self, stock_data, summary, company_profile=None):
        ...
        if self.config["output"]["use_database"]:
            ...
            try:
                ...
                company_name = company_profile['companyName'] if company_profile else stock_data.get('companyName', symbol)
                ...
                cur.execute(query, (
                    symbol, stock_data['window_start'], stock_data['pct_change'], 
                    prev_avg_close, avg_close, company_name, sector, industry, summary,
                    summary, prev_avg_close, avg_close
                ))
                conn.commit()
                logger.info(f"Summary for {symbol} saved to database")
            except Exception as e:
            ...
            output_file = os.path.join(output_dir, f"summary_{symbol}_{date_str}.txt")
